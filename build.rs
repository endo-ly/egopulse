...
fn ensure_web_assets() {
    ...
    let skip_web_build = env::var_os("EGOPULSE_SKIP_WEB_BUILD").is_some();

    if web_assets_need_rebuild(source_dist) {
        if skip_web_build {
            panic!(
                "web assets are missing or stale, but EGOPULSE_SKIP_WEB_BUILD is set; regenerate web/dist before building"
            );
        }
        build_web_assets();
    }
    ...
    let web_assets_source = format!(
        "static WEB_ASSETS: include_dir::Dir<'_> = include_dir::include_dir!({:?});\n",
...
fn assert_web_assets_ready(dist_dir: &Path) {
    ...
    panic!(
        "web asset directory {} is incomplete; expected index.html and at least one JavaScript bundle in assets/",
        dist_dir.display()
    );
}

fn has_required_web_assets(dist_dir: &Path) -> bool {
    dist_dir.join("index.html").is_file() && has_js_bundle(&dist_dir.join("assets"))
}
