...
flowchart TD
    ...
    T --> DB["Database（ドメイン別 module）"]
    DB --> TR["turn_runs<br/>Turn 状態機械"]
    DB --> CS["chats / messages / sessions<br/>会話順序・revision CAS"]
    DB --> TL["tool_calls<br/>Tool 実行台帳"]
    T --> G["ConfigManager<br/>immutable snapshot"]
    T --> P["ProviderRegistry"]
    T --> K["ToolRegistry"]
```

Turn・会話・Tool の各永続化は、独立した Repository 型ではなく `Database` のドメイン別 module（`turn.rs` / `chat.rs` / `tool.rs`）として実装する。境界は module と型付き API で表現し、汎用 event や同じ事実の重複保存は導入しない。
...
| `Database`（`turn` module） | `turn_runs`の作成、重複受付防止、状態遷移、失敗理由保存                       |
| `Database`（`chat` module） | message順序発行、revision CAS、messageとsession snapshotの原子的更新 |
| `Database`（`tool` module） | Tool claim、状態更新、input整合確認、結果再利用、uncertain判定             |
| `ConfigManager`           | Config検証、revision／fingerprint発行、immutable snapshotの提供    |
...
## 6. Work Package 2 — 会話ドメイン（Database `chat` module）への書込み集約
...
会話変更を`Database`の会話ドメイン（`chat` module）の型付き API へ集約する。
...
Channel Logなどsessionを持たないmessageについても、会話ドメイン（`chat` module）内の専用APIを通す。
...
既存Turnが`completed`なら保存済み最終結果を利用する。実行中なら新しい実行を開始せず、「処理中である」旨を終端応答として返す。`failed` / `uncertain` / `cancelled` で停止している場合も再実行せず、その旨を終端応答で伝える。いずれの受付結果も呼び出し元へ明確に終端し、Web `done` イベントやチャネル応答として観測される。
...
3. user inputの保存と`accepted → input_committed`遷移を同一 SQLite transaction で行う（message insert・session snapshot更新・`chats.revision`/`next_message_seq` 更新・`turn_runs.input_message_id` 記録）。
4. model iterationを開始する。
...
起動時に未端末 Turn を fail-stop で停止する。再開の安全性を証明できない Turn を推測で再開せず、すべて停止状態へ移す。同じ Turn の再受付は、停止状態に応じて「処理中」「以前の処理は停止した」旨の終端応答を返す（自動再開はしない）。

| 状態                | 起動時処理                                              |
| ----------------- | -------------------------------------------------- |
| `accepted`        | `failed`（input 未 commit。再受付で安全に再開可能）         |
| `input_committed` | `failed`（model 未実行。再受付で安全に再開可能）              |
| `model_pending`   | `uncertain`（外部出力が及んでいる可能性あり）                 |
| `model_completed` | `uncertain`（外部出力が及んでいる可能性あり）                 |
| `tools_pending`   | `uncertain`（Tool 副作用が及んでいる可能性あり）            |
| `tools_completed` | `uncertain`（Tool 副作用が及んでいる可能性あり）            |
| `completed`       | 端末状態。変更しない                                  |
| `failed`          | 端末状態。変更しない                                  |
| `uncertain`       | 端末状態。変更しない                                  |
| `cancelled`       | 端末状態。変更しない                                  |

起動時の fail-stop は `config_fingerprint` に関わらず適用する。`config_fingerprint` は受付時の Config 同一性記録として保持するが、再開判定には用いない。`running` の Tool は同時にすべて `uncertain` へ移し、`pending` に戻さない。
...
Tool実行前に、Tool ドメイン（`tool` module）の `claim_tool_execution` を呼び出す。
...
プロセス起動時に`running`のToolが残っていた場合、idempotency 分類に関わらずすべて`uncertain`へ遷移する。Turn は起動時に fail-stop するため再開されず、`running` の Tool が再 claim されることはない。`pending`（実行開始前）の行はそのまま残す。安全な Tool 再試行は、Turn 再開とともに導入する。
...
* `revision`（同一 revision は同一 Config 内容を保証する識別番号）
...
ConfigManager は起動時に 1 つの immutable snapshot を構築する。同一プロセス生存中は snapshot を固定し、Turn はその snapshot を共有する。
...
過去Config snapshotそのものをDBへ保存しない。

起動時の Turn 復旧は fail-stop であり、`config_fingerprint` の一致・不一致に関わらず未端末 Turn を停止状態へ移す。`config_fingerprint` は受付時の Config 同一性記録として保持するが、再開判定には用いない。

| 条件             | 扱い                          |
| -------------- | --------------------------- |
| 未完了 Turn | `accepted` / `input_committed` は `failed`、それ以上は `uncertain` へ停止。fingerprint に関わらず自動再開しない |
| 端末 Turn | 変更しない |
...
TurnRuntime
├── ConfigManager
├── Database（会話・Turn・Tool のドメイン別 module）
├── ProviderRegistry
└── ToolRegistry
```

Turn・会話・Tool の永続化は `Database` のドメイン別 module（`turn` / `chat` / `tool`）として実装し、独立した Repository 型は作らない。
...
* Conversation永続化が会話ドメイン（`chat` module）以外から行われない。
* Turn状態更新がTurnドメイン（`turn` module）以外から行われない。
* Tool状態更新がToolドメイン（`tool` module）以外から行われない。
...
* すべての会話変更が会話ドメイン（`chat` module）を経由する。
...
### Package 2：Database ドメイン別 module と会話集約

* Turn ドメイン（`turn` module、`turn_runs`）
* 会話ドメイン（`chat` module）
* Tool ドメイン（`tool` module、実行台帳拡張）
...
* [ ] Turn executorが`AppState`を直接参照しない。
* [ ] Conversation writeが会話ドメイン（`chat` module）へ集約されている。
* [ ] Tool状態更新がToolドメイン（`tool` module）へ集約されている。
...
Phase 3では、Phase 2で確立した`TurnRuntime`と `Database` ドメイン別 module（会話・Turn・Tool）を前提に、次を扱う。
