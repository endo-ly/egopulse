...
この仕様では StackChan を最初の接続先として扱うが、EgoPulse 側の抽象は `stackchan` ではなく `voice` とする。StackChan は voice channel に接続する具体的な voice surface の一つであり、将来的な別マイク、WebRTC、スマートフォン、別デバイス入力と同じ枠で扱う。
...
| device bridge | 音声デバイス固有の I/O を扱う外部プロセス。初期実装では `stackchan-bridge` |
...
| `metadata` | `object` | `{}` | STT timing などの観測情報。LLM への入力本文には混ぜない |
...
| `voice_channel_disabled` | 404 or 503 | `channels.voice.enabled` が false |
...
`metadata` は observability 用にログや telemetry に残してよいが、初期実装では system prompt や user message へ注入しない。ユーザー入力として LLM に渡す本文は原則 `text` のみとする。

EgoPulse の履歴には、通常の user/assistant message として保存する。

## Channel Adapter

初期実装では `voice` の `ChannelAdapter::send_text()` は no-op でよい。bridge 呼び出しの HTTP response で応答テキストを返し、TTS と playback は bridge が担当する。

ただし、将来的に EgoPulse 起点の自発発話や `agent_send` の応答を音声再生したくなった場合に備え、`voice` channel adapter の存在は持たせる。
...
channels:
  voice:
    outputs:
      stackchan:
        speak_url: "http://127.0.0.1:8787/speak"
        auth_token:
          source: env
          id: STACKCHAN_BRIDGE_AUTH_TOKEN
```

この段階では実装しない。初期実装では inbound request/response の同期 turn を主経路とする。
...
- request `metadata` は信頼しない。観測用データとして扱い、system prompt には混ぜない。
...
これらは voice channel の基本 turn が安定してから、別仕様として扱う。
