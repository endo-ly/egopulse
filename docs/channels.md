...
6. [TUI](#6-tui)
7. [CLI](#7-cli)
8. [Webhook Trigger](#8-webhook-trigger)
...
- Webの ordinary message はRESTとWebSocketの入口にかかわらず共通 TurnScheduler に投入され、同一 session では FIFO で実行される。active run と同一 session の Tool 実行中は接続元に依存せず follow-up として durable staging され、Tool Result 後に同じinteractionへ引き継がれる。共有observer interaction内の途中Turnの `done` / `error` は非終端として同じRunHubへ流れ、最後のTurnの `terminal: true` でだけ購読を終了する。ACKが不明な送信の明示的な再送では、未変更の同じdraftに限り同じ durable `request_id` / `request_key` を使い、WebSocketのRPC IDはattemptごとに変える。draft編集や別sessionの送信は新しいidentityを使い、別本文の同じrequest identity再利用は `409 Conflict` として拒否される。slash command はdurableな未完了Turnを持つsessionでは `busy` となり、busy確認からcommand完了まで通常入力と同じsession排他を保持する
...
長時間ターン（`web_fetch` 連打・重い `bash`・多数のツール呼び出しなど）で「何をしているか」を可視化する。チャネル個別の `tool_progress: true` 設定でのみ有効（デフォルト `false`＝完全互換、ノイズゼロ）。

- **A3 遅延型**: ターン開始から 5 秒未満の速いターンは進捗を投稿しない。5 秒を超えてもツールが実行中の場合のみ表示を開始する
- **B2 編集式・累積ログ**: 1 メッセージを投稿し、ツールイベントごとに `PATCH /messages/{id}` で本文を編集更新する。完了後も累積ログとして残置する
- **ライフサイクル**:
  - 単一の長時間ツールが 5 秒閾値を跨いだ時点で遅延タイマーが初回投稿を発火する
  - 完了ツールは `✓ <name> (<duration>s)`、エラーは `✗ <name> (<duration>s) エラー`、実行中は `... <name>` で表示
  - ツールコールに先行するアシスタントの発言（narration）があれば `💬 <text>` で表示し、ツール行の上に時系列で挿入する。最終応答の断片は別メッセージで送られるため進捗には含めない（重複回避）
  - 編集は最小間隔 800ms で間引きし、429 時は Retry-After で 3 回リトライする
  - イベントストリーム終了（EOF / 最終応答）で確実に close する
- **本文は 2000 文字制限**に合わせて最新行を優先して打ち切る。**ツール引数・結果 preview は Discord 進捗本文に一切含めない**（公開チャネルへの情報漏洩防止）
- 最終応答は進捗メッセージとは別に新規投稿される。進捗の編集遅延・失敗が最終応答送信を長時間ブロックしないよう、進捗 coordinator の終了待ちには短いタイムアウトを設ける
...
長時間ターン（`web_fetch` 連打・重い `bash`・多数のツール呼び出しなど）で「何をしているか」を可視化する。チャット個別の `tool_progress: true` 設定でのみ有効（デフォルト `false`＝完全互換、ノイズゼロ）。詳細な挙動（A3 遅延型 × B2 編集式・累積ログ）は [Discord §ツール進捗表示](#3-discord) と共通。

- **編集 API**: `editMessageText` で本文を更新する（`sendMessage` で初回投稿し `result.message_id` を保持）
- **本文は 4096 文字制限**に合わせて最新行を優先して打ち切る
- **ツール引数・結果 preview は Telegram 進捗本文に一切含めない**（公開チャットへの情報漏洩防止）
- Telegram edit は 48 時間以内のみ対応だが、ターン中（数秒〜数十秒）のため実質無害。編集失敗時は warn ログのみで最終応答送信を妨げない
...
- **アクティブターン領域**: ストリーミング本文、iteration、ツールカードを表示
- **入力コンポーザ**: 1〜8 行の入力ボックス。入力履歴とブラケットペーストに対応
- **状態行**: セッション、agent、model、実行状態、終了操作を表示
- **セッションオーバーレイ**: `/sessions` で保存済みセッションを表示し、`j/k` または上下キーで移動、`Enter` で切替、`n` で新規、`Esc` で閉じる
...
- `egopulse --session <NAME>` は指定セッションを開く
...
- **メッセージ受信**: 位置引数 `PROMPT` またはパイプされた stdin
- **応答**: 装飾なしの応答本文を stdout に出力
- **ログ**: stderr に出力

### 制約

- stdin は非 TTY の場合だけ読み取る
- ストリーミング非対応
- ファイル添付非対応
...
Webhook は会話チャネルではなく、外部イベントの trigger として扱う。`ChannelAdapter` を実装せず、`ChannelRegistry` にも登録されない。

### 位置づけ

受信した Webhook payload を整形し、receiver ごとに設定した target channel（Discord / Telegram / Web）上の agent turn として `TurnScheduler` へ投入する。応答は target channel の通常配送経路で送信される。
...
Webhook payload は Channel Log に保存しない。通常の agent turn input として target session の履歴に保存される。
