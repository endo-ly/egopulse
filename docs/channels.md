...
- **エージェント選択**:
  ...
  - Single-Agent チャネル（`multi_agent: false`）→ `channels[channel_id].agents[0]`
  - Multi-Agent Room（`multi_agent: true`）→ @mention されたボットに紐づくエージェントを特定。mention なし → 応答なし（Channel Log のみ保存）
- **メッセージ振り分け**:
