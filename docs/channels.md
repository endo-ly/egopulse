...
- **メンション要件**: `require_mention: true` のチャンネルでは Bot への @mention なしで応答しない
...
- `require_mention: true` のチャンネルでは Bot への @mention なしで応答しない
- Slash command は `channels` に含まれるチャンネルであれば常に有効（mention 不要）
