...
registry に登録される tool は次の通り。`agent_send` は Discord または Telegram 有効時のみ登録される。
...
- `agent_send` — Discord または Telegram チャネル有効時のみ
...
- 挙動:
  - Discord または Telegram チャネルが設定されている場合のみ登録される（CLI / Web / TUI ではツールが露出しない）
  - 非対応チャネル (Discord / Telegram 以外) からの実行時にもエラーを返す（ランタイムガード）
  - 自己送信 (`to == 自分自身`) は禁止
