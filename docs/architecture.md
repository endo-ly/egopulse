...
| **Turn Output Observer** | `runtime/turn/observer.rs` | Local APIなどのtransport adapterへTurn eventとcompletionを配送する。live observerはメモリに保持し、durable TurnにはClient-ownedという配送ポリシーだけを保存する。terminalなTurnから複数のstaged follow-upを昇格する場合も、同じclient interactionへイベントを集約し、全Turnの完了後にcompletionを返す |
