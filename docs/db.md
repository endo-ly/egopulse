...
└── turn_runs                — Turn実行状態機械（Phase 2 のdurable turn lifecycle）
```
...
| revision | INTEGER | NOT NULL DEFAULT 0 | 会話変更ごとに増加する整数CAS（Phase 2）。timestamp を競合判定から排除 |
| next_message_seq | INTEGER | NOT NULL DEFAULT 1 | 次に発行する chat 内 message sequence（Phase 2） |
...
| seq | INTEGER | nullable | chat 内の単調増加 sequence（Phase 2）。因果順序の根拠。未割当は NULL |
| turn_id | TEXT | nullable | 所属 Turn（Phase 2 `turn_runs`）。legacy 行は NULL |
| parent_message_id | TEXT | nullable | Tool Result 等が参照する親 message（Phase 2） |
...
| updated_at | TEXT | NOT NULL | 監査時刻（RFC3339）。Phase 2 以降は競合判定に使用せず整数 `chats.revision` を使用 |
| snapshot_through_seq | INTEGER | NOT NULL DEFAULT 0 | `messages_json` へ反映済みの最終 message seq（Phase 2）。`base + seq>snapshot_through` でLLM contextを構築 |
...
| timestamp | TEXT | NOT NULL | RFC3339 タイムスタンプ |
| turn_id | TEXT | nullable | 所属 Turn（Phase 2 `turn_runs`）。legacy 行は NULL |
| state | TEXT | NOT NULL DEFAULT 'pending' | Tool実行状態（Phase 2）。`pending`/`running`/`succeeded`/`failed`/`uncertain` |
| input_hash | TEXT | nullable | canonical input の hash（Phase 2）。同一call IDでのinput整合確認 |
| idempotency_class | TEXT | nullable | 再実行可能性分類（Phase 2）。`read_only`/`idempotent`/`non_idempotent` |
| idempotency_key | TEXT | nullable | idempotent Tool の重複排除 key（Phase 2） |
| started_at | TEXT | nullable | 実行開始時刻（Phase 2） |
| finished_at | TEXT | nullable | 完了時刻（Phase 2） |
| error_kind | TEXT | nullable | 失敗分類（Phase 2） |
| error_message | TEXT | nullable | sanitized error 概要（Phase 2） |
...
- Phase 2 では新規台帳テーブルを作らず、本テーブルを Tool 実行台帳として拡張する（`ToolExecutionRepository` が claim・状態遷移・結果再利用を担う）
...
Phase 2 で導入する Turn 実行の状態機械。受付・入力保存・model iteration・Tool 実行・完了・失敗・uncertain のライフサイクルを永続化し、重複受付防止と安全な再試行・復旧判断を可能にする。詳細は [リポジトリ安定化 Phase 2](../plan/plan-repository-stabilization-phase2.md) を参照。
...
| config_revision | INTEGER | NOT NULL DEFAULT 0 | Turn 開始時の Config revision（Phase 2 WP5） |
| config_fingerprint | TEXT | nullable | Config 内容の識別 hash（Phase 2 WP5） |
...
**設計ポイント**:
- 状態遷移は Rust enum と中央定義した transition rule で管理し、許可されていない遷移は DB 更新前に拒否する（Phase 2 WP3/WP4）
- `output_published` が真の Turn は partial output を外部公開済みのため自動 retry しない
- WP1 ではテーブル作成・CHECK 制約のみ。CRUD と状態遷移ロジックは WP2〜WP4 で追加する
...
| Turn 状態（`turn_runs.state`） | — | 10種: `accepted`, `input_committed`, `model_pending`, `model_completed`, `tools_pending`, `tools_completed`, `completed`, `failed`, `cancelled`, `uncertain`（Phase 2） |
| Tool 実行状態（`tool_calls.state`） | — | 5種: `pending`, `running`, `succeeded`, `failed`, `uncertain`（Phase 2） |
...
#### v12: durable turn lifecycle + integer seq/revision（Phase 2 WP1）

Phase 2 の Turn 永続化導入。新規 `turn_runs` テーブル（CHECK 制約付き state、`UNIQUE(chat_id, request_key)`）、`chats`/`messages`/`sessions`/`tool_calls` への拡張カラム追加、legacy backfill を同一 transaction で行う。全面 Event Sourcing は導入せず、既存テーブルを正本として拡張する。
...
| テーブル数 | 8（`chats`, `messages`, `sessions`, `llm_usage_logs`, `turn_runs`, `db_meta`, `schema_migrations`、Phase 2 追加分を含む概念数） |
...
`secret.db` は `egopulse.db` のサブセット。通常 DB に存在する `tool_calls`, `sleep_runs`, `sleep_run_steps`, `sleep_step_checkpoints`, `pulse_runs`, `episode_events`, `episode_rollups`, `memory_snapshots` は含まない。Phase 2 では秘匿会話も同一の会話順序・競合制御・Turn lifecycle を持つため、`chats`/`messages`/`sessions` 拡張カラムと `turn_runs` を追加する（`tool_calls` は不在のまま）。
