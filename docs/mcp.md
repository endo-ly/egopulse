...
MCP tool は built-in tool と異なり出力サイズを自己規制しないため、`100KB`（`DEFAULT_MAX_BYTES`）を超える場合は head 側を切断し `... (output truncated to 102400 bytes)` を付加する。built-in tool は各自が同限値で切断済みであるため、このセーフティネットは実質 MCP にのみ作用する。
