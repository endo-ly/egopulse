...
| `status` | サービスの稼働状態を表示（`--json` で JSON 出力） |
...
### 1.5 `egopulse gateway status`

サービスの稼働状態を表示する。サービスが実行中の場合は `/ready` エンドポイントからライブ情報を取得して表示する。`--json` フラグで JSON 形式の出力が得られる。サービスが停止中の場合は `systemctl --user status` の出力にフォールバックする。
