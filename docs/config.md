...
| `DISCORD_BOT_TOKEN` | `channels.discord.bots.*.token` | `string` | SecretRef `{ source: env, id: DISCORD_BOT_TOKEN }` で解決（§4 参照）。Layer 2 の直接オーバーライドは非対応 |
| `TELEGRAM_BOT_TOKEN` | `channels.telegram.bots.*.token` | `string` | SecretRef `{ source: env, id: TELEGRAM_BOT_TOKEN }` で解決（§4 参照）。Layer 2 の直接オーバーライドは非対応 |
...
| `channels.telegram.bots` | Bot 定義の更新 |
| `channels.telegram.channels` | チャットアクセス制御・メンション要件の変更 |
...
| `channels.telegram.bots.<bot_id>.token` | 返却なし |
