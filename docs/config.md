...
| フィールド | 型 | 必須 | デフォルト | 説明 |
|---|---|---|---|---|
| `default_agent` | `string` | 任意 | `"default"` | エージェント未指定時に使用される Agent ID。`agents` マップ内に存在する必要がある |
| `agents` | `map` | 任意 | `{default: {...}}` | エージェント定義マップ。キーは AgentId |

`agents` マップの各エージェント定義:

| フィールド | 型 | 必須 | 説明 |
|---|---|---|---|
| `label` | `string` | 任意 | エージェントの表示名 |
| `model` | `string \| null` | 任意 | エージェント固有のモデルオーバーライド。`null` の場合チャネル/グローバル設定にフォールバック |
| `provider` | `string \| null` | 任意 | エージェント固有のプロバイダーオーバーライド。`null` の場合チャネル/グローバル設定にフォールバック |
| `discord.bot_token` | `string \| SecretRef \| null` | 任意 | 1 BOT = 1 Agent 運用時の Discord Bot トークン。SecretRef 使用可能。秘匿フィールド |
| `discord.allowed_channels` | `[u64]` | 任意 | この Agent/Bot が応答する Discord ギルドチャンネル ID。空の場合はギルドメッセージを全拒否（DM は常に許可） |

`channels.discord.bot_token` は単一Bot互換のチャネル設定であり、マルチBot運用では `agents.<id>.discord.bot_token` を使用する。
...
| `agents.<id>.discord.bot_token` | Agent別 Discord Bot 認証の再確立 |
...
| `agents.<id>.discord.bot_token` | 返却なし |
...
| **エージェント定義** | | | | |
| `default_agent` | R/W | なし | なし | なし |
| `agents` の内容 | R/W | なし | なし | W（現在Agentの `/provider` `/model`） |
| `agents.<id>.discord.bot_token` | R/W | なし | なし | なし |
| `agents.<id>.discord.allowed_channels` | R/W | なし | なし | なし |
| **プロバイダー定義** | | | | |
...
| `channels.discord.provider` | R/W | なし | R/W | W（`/provider --scope discord`） |
| `channels.discord.model` | R/W | なし | R/W | W（`/model --scope discord`） |
...
| `channels.telegram.provider` | R/W | なし | R/W | W（`/provider --scope telegram`） |
| `channels.telegram.model` | R/W | なし | R/W | W（`/model --scope telegram`） |
