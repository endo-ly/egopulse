...
| `default_agent` | `string` | 任意 | グローバル `default_agent` | この Bot のデフォルト応答エージェント |
...
| `model` | `string \| null` | 任意 | エージェント固有のモデルオーバーライド |
| `provider` | `string \| null` | 任意 | エージェント固有のプロバイダーオーバーライド |

Discord Bot の設定は `channels.discord.bots.<bot_id>` 配下で行う（[§2.4](#24-discord-チャネルchannelsdiscord) 参照）。エージェントごとに Bot トークンを持たせる旧形式 (`agents.<id>.discord.*`) は非推奨。
...
| `channels.discord.bots.<bot_id>.token` | R/W | W（`DISCORD_BOT_TOKEN`） | なし | なし |
| `channels.discord.provider` | R/W | なし | R/W | W（`/provider --scope discord`） |
| `channels.discord.model` | R/W | なし | R/W | W（`/model --scope discord`） |
| `channels.discord.bots.<bot_id>.allowed_channels` | R/W | なし | なし | なし |
| `channels.discord.bots.<bot_id>.channel_agents` | R/W | なし | なし | なし |
| **Telegram チャネル** | | | | |
