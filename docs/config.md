...
12. [ConfigManager と immutable Snapshot](#12-configmanager-と-immutable-snapshot)
...
> これらのフィールドは [ConfigManager snapshot](#12-configmanager-と-immutable-snapshot) へ集約されている。runtime は起動時に一度だけ snapshot を構築し、以後の Turn は全てその snapshot を参照する。ファイル変更を反映するには、runtime の再起動が必要（§12 参照）。
...
`ConfigSnapshot` はある時点の validated config を不変オブジェクトとして保持する。

| フィールド | 役割 |
|---|---|
| `revision` | 単調増加する世代番号（1, 2, 3, ...） |
| `fingerprint` | Config 内容の識別 hash（SHA-256）。ファイル内容または主要 field から算出 |
| `config` | validated `Config` |

### 12.2 Turn での固定

Turn 開始時に `Arc<ConfigSnapshot>` を取得し、Turn 完了まで保持する。Turn 内部から Config ファイルを同期再読込しない。

`turn_runs` には snapshot の `revision` と `fingerprint` を保存する。

### 12.3 再起動後の fingerprint 比較

過去の Config snapshot は DB へ保存しない。未完了 Turn を再開する際は、現在 Config の fingerprint と `turn_runs.config_fingerprint` を比較する。

| 条件 | 扱い |
|---|---|
| fingerprint 一致 | 他の再開条件を満たせば再開可能 |
| fingerprint 不一致 | 自動再開せず `uncertain`（[session-lifecycle.md §9](./session-lifecycle.md#93-crash-recovery) 参照） |
...
| 概念 | 正本 | 役割 |
|---|---|---|
| DB schema version | `db_meta.schema_version`（[db.md](./db.md#db_meta)） | SQLite スキーマのマイグレーション版数。未対応版は起動を拒否 |
| Config revision | `ConfigSnapshot.revision` | 設定世代の単調増加番号。同一 revision は同一 Config 内容を保証 |
| Pulse definition version | `PULSE.md` front matter `version`（[pulse.md](./pulse.md)） | Pulse 定義フォーマットの版数。未対応版は読込を拒否 |
