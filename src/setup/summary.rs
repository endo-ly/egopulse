...
pub(crate) fn save_config_from_inputs(
    inputs: &SetupInputs,
    original_yaml: Option<&yaml_serde::Value>,
    config_path: &Path,
) -> Result<(Option<String>, Vec<String>), String> {
...
mod tests {
    ...
    use super::save_config_from_inputs;
...
"#;
    ...
    fn save_config_persists_agent_label() {
        ...
        save_config_from_inputs(&inputs, None, &config_path).expect("save config");
    ...
    fn save_config_reuses_existing_web_auth_token() {
        ...
        save_config_from_inputs(&inputs, Some(&parsed_yaml), &config_path).expect("save config");
