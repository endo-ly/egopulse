...
1. `--config <PATH>` で指定したYAML
2. `~/.egopulse/egopulse.config.yaml` 自動検出
...
| `egopulse status` | 最後の起動ステータスを表示（MCP、チャネル、provider） | 不要 |
...
CLI / TUI チャットでは以下の LLM 切替コマンドを利用できます。

```text
/providers
/provider
/provider openrouter --scope web
/provider reset --scope discord
/models
/model gpt-5
/model reset --scope web
```
...
`egopulse run` の最後の起動ステータスを表示。プロセス停止後も参照可能。

```bash
egopulse status           # ASCII テーブルで表示
egopulse status --json    # 生 JSON で表示
```
...
ステータスは起動時に `%h/.egopulse/status.json` に書き出される（`egopulse gateway status` はOSレベルのプロセス状態を表示する別コマンド）。
